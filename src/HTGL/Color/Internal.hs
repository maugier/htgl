...
module HTGL.Color.Internal where
...
instance Colorful (Text) where
    colorful = pure

instance Colorful a => Colorful [a] where
    colorful xs = mconcat (("[" : intersperse "," (colorful <$> xs)) ++ ["]"])

showStyle :: (Show s) => s -> Style -> Colored Text
showStyle = withStyle . fromString . show
